...
void pyplot::plot_dots(vector<complex<float>> in) {
    ...
    vector<float> x(in.size());
    vector<float> y(in.size());
//    transform(in.begin(), in.end(), x.begin(), [](auto x) -> float { return real(x); });
//    transform(in.begin(), in.end(), y.begin(), [](auto x) -> float { return imag(x); });
    plot_dots(x, y);
}
